...
public static partial class Utils
{
    ...
    public static bool Contains_<T>(this List<T> list, T item) where T : class
    {
    ...
    public static bool Any_<T>(this List<T> list, Func<T, bool> predicate)
    {
        var count = list.Count;
        for (int i = 0; i < count; i++) {
            if (predicate(list[i]))
    ...
    public static T FirstOrDefault_<T>(this List<T> list, Func<T, bool> predicate)
    {
        var count = list.Count;
        for (int i = 0; i < count; i++) {
            var item = list[i];
